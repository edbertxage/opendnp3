...
{
        string outstationAddress = "127.0.0.1";
        if(argc>=2){
	  outstationAddress = argv[1];
	}
	...
	auto channel = manager.AddTCPClient("tcpclient", FILTERS, ChannelRetry::Default(), { IPEndpoint(outstationAddress, 20000) }, "0.0.0.0", PrintingChannelListener::Create());
